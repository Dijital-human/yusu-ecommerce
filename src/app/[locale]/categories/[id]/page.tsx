...
export default function CategoryDetailsPage() {
  ...
  const fetchCategory = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Handle API response structure / API cavab strukturunu idarə et
        const categoryData = data.data || data;
        setCategory(categoryData);
        setPriceRange({ min: 0, max: categoryData.maxPrice || 1000 });
        setSelectedPriceRange({ min: 0, max: categoryData.maxPrice || 1000 });
      } else {
        setError("Category not found / Kateqoriya tapılmadı");
      }
    } catch (error) {
      console.error("Error fetching category:", error);
      setError("Failed to fetch category / Kateqoriya alına bilmədi");
    }
  };
  ...
  const fetchProducts = async () => {
    try {
      ...
      // Normalize images field - convert string to array if needed / Images field-ini normallaşdır - lazım olduqda string-i array-ə çevir
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            ...
            <Badge className="bg-blue-100 text-blue-800">
              {category.productCount ?? category._count?.products ?? 0} products / məhsul
            </Badge>
          </div>
